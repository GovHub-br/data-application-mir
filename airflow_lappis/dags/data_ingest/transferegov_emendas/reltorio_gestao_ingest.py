...
def api_relatorio_gestao_especial_dag() -> None:
    ...
    def process_chunk(chunk: list) -> None:
        ...
        for plano_id in chunk:
            ...
            # Chama o novo método criado no cliente
            relatorios = api.get_all_relatorio_gestao_especial_by_plano_acao(plano_id)

            if not relatorios:
                # Não há relatórios para este plano -> pula
                continue

            # Adiciona metadados aos registros
            for item in relatorios:
                # Garante que o ID do plano está no registro (caso a API não retorne no corpo)
        ...
        # Remove duplicatas locais usando a PK (id_relatorio_gestao)
        unique = {}
        for row in all_relatorios:
            # Baseado na imagem do Swagger, a PK parece ser id_relatorio_gestao
            # Se quiser garantir unicidade por plano, pode usar tupla (id_plano_acao, id_relatorio_gestao)
        ...
        all_relatorios_unique = list(unique.values())
